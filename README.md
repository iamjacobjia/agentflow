...
That wrapper now runs both the raw shell-level Kimi toolchain check and the bundled `agentflow toolchain-local`, `inspect`, `doctor`, `smoke`, `run`, and `check-local` commands before it moves on to the external custom-pipeline `doctor`, `inspect`, `check-local`, and `run` paths for `bootstrap: kimi`, explicit `shell_init: kimi`, and explicit `target.shell` Kimi wrappers. The bundled `run` leg specifically validates the stdout/stderr wrapper contract for the repo's own smoke pipeline, so regressions in `agentflow run` cannot hide behind a still-green `smoke` or custom-pipeline path. It also prints whether `~/.bash_profile`, `~/.bash_login`, or `~/.profile` is supplying the bash login startup path before it verifies `kimi`, `codex`, and `claude`, reports whether Codex auth is coming from `OPENAI_API_KEY`, Codex CLI login, or both in that shared Kimi-backed shell, and shows the resolved `codex` / `claude` executable paths so it is easier to catch mixed Node/npm installs.
Those maintainer scripts now apply the same timeout guard to every live `agentflow` invocation in the stack, including the bundled `toolchain-local`, `inspect`, `doctor`, `smoke`, `run`, and `check-local` steps plus the external `doctor`, `inspect`, `check-local`, and `run` paths. Override that stack-wide budget with `AGENTFLOW_LOCAL_VERIFY_TIMEOUT_SECONDS`; when it is unset, the helper falls back to `AGENTFLOW_DOCTOR_TIMEOUT_SECONDS` if you already exported it, otherwise it uses a 60-second default that is large enough for the real local Codex plus Claude-on-Kimi runs.
