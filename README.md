...
Before those checks run, `agentflow toolchain-local` also prints the active bash login startup chain, for example `~/.bash_profile -> ~/.profile -> ~/.bashrc` or `~/.profile -> ~/.bashrc`, so it is easier to confirm which file actually supplied the shared Kimi bootstrap on your machine.
